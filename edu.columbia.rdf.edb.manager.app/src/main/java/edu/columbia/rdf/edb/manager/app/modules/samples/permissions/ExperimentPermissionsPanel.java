...
public class ExperimentPermissionsPanel extends ModernPanel {

	private static final long serialVersionUID = 1L;

	private Connection mConnection;

	private ModernRowTable mTable = new ExperimentPermissionsTable();

	private ModernButton mNewButton = new ModernButton("New...");

	private ModernButton mUpdateButton = 
			new ModernButton("Update", UIService.getInstance().loadIcon(RunVectorIcon.class, 16));
	...
	private PersonCombo mPersonsCombo;

	private SelectWhereQuery mExperimentPermissions;

	private ExperimentPermissionsTableModel mModel;
	
	private boolean mSelectAll = true;


	public ExperimentPermissionsPanel(Connection connection, MainManagerWindow window) throws SQLException {
		mConnection = connection;
		mWindow = window;

		Box box = HBox.create();
		
		mPersonsCombo = new PersonCombo(connection);
		...
		mNewButton.addClickListener(new ModernClickListener() {
			...
			public void clicked(ModernClickEvent arg0) {
				try {
					...
					ModernMessageDialog.createDialog(mWindow, "There was an error adding the user", MessageDialogType.WARNING);
				}
			}});
		...
		mExperimentsQuery = mQuery.select("id", "name")
				.from("experiments");
		
		mExperimentPermissions = mQuery.select("id", "person_id", "experiment_id")
				.from("experiment_permissions")
				.where("person_id");
		
		mRemoveExperimentPermissions = mQuery.delete()
				.from("experiment_permissions")
				.where("person_id");
		
		mAddPermissionQuery = mQuery.insert("experiment_permissions")
				.cols("experiment_id", "person_id");
	...
	private void refresh() throws SQLException {
		...
		mModel = new ExperimentPermissionsTableModel(results, permissions);
	...
	private void editPerson() throws SQLException, CryptographyException {
		...
		ExperimentPermissionsDialog dialog = new ExperimentPermissionsDialog(mWindow,
				personTable);
