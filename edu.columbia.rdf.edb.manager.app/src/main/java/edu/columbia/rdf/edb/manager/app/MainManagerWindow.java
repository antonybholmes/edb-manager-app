...
public class MainManagerWindow extends ModernRibbonWindow
  ...
  public MainManagerWindow(EDBLogin login)
  ...
  public final void createRibbon() {
     ...
     * 
     * button = new RibbonLargeButton2("Update",
     * UIResources.getInstance().loadIcon(RunVectorIcon.class, 24));
     * button.addClickListener(new ModernClickListener() {
     * 
     * @Override public void clicked(ModernClickEvent e) { try {
     * ExperimentPermissions.main(null); SamplePermissions.main(null); } catch
     * (SQLException e1) { e1.printStackTrace(); } catch (ParseException e1) {
     * e1.printStackTrace(); } }});
     * 
     * 
  ...
  private void gep() throws SQLException, IOException, ParseException {
    Path dir = FileDialog.openDir(this,
  ...
  */
  ...
  public void loadPeaksJson() throws SQLException, IOException {
    ...
    try {
      ...
      Path dir = PathUtils.getPath(
          "/ifs/scratch/cancer/Lab_RDF/abh2138/ChIP_seq/data/samples/hg19/rdf/json");

      List<Path> files = FileUtils.findAll(dir, "json");

      PreparedStatement findStatement = connection
          .prepareStatement("SELECT samples.id FROM samples where name LIKE ?");
          ...
          if (matcher.find()) {
            ...
            if (id != -1) {
              ...
              String s = sql.replace("?1", Integer.toString(id))
                  .replace("?2", name).replace("?3", Io.getHead(file));
