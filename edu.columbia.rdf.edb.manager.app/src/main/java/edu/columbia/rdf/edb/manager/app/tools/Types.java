 ...
 */
public class Types {
	public static Type createType(Connection connection, 
			String type, 
	...
	/**
	 * Creates a type in one of the type tables which
	 * all have the same structure so the can be
	 * addressed with the same code apart from
	...
	public static Type createType(Connection connection, 
		...
		/*
		DatabaseResultsTable query = new TableQuery(connection)
				.select("id", "name")
				.from(type)
				.where("name")
				.values(name)
