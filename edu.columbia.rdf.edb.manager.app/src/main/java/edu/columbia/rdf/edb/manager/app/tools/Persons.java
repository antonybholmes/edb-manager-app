...
public class Persons {
	public static final String LOGIN_SQL = 
			"SELECT login_persons.id, login_persons.user_name, login_persons.password_hash_salted, login_persons.salt";
	
	public static void main(String[] args) throws SQLException, CryptographyException, SQLException, ParseException {
		...
		try {
			...
			//String key = LoginService.createSessionKey(connection, personId, ipAddress);
			
			System.err.println(user + " " + person.getId());

			/*
			personId = createPerson(connection,
					"Guest", 
					"Guest",
					"Columbia University",
					"123-456-7890",
	...
	public static Person createPersonFromIdf(Connection connection,
			java.nio.file.Path idfFile) throws SQLException, IOException, SQLException, ParseException {
	...
	private static String createApiKey(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT api_key_persons.key FROM api_key_persons WHERE api_key_persons.person_id = ?");
		...
		if (oldKey != null) {
			statement = 
					connection.prepareStatement("DELETE FROM api_keys WHERE key = ? AND ip_address = ?");
		...
		statement = 
				connection.prepareStatement("DELETE FROM api_key_persons WHERE person_id = ?");
		...
		statement = connection.prepareStatement("INSERT INTO api_key_persons (person_id, key) VALUES (?, ?)");
		...
		statement = connection.prepareStatement("INSERT INTO api_keys (key, ip_address) VALUES (?, ?)");
	...
	/**
	 * Map an ip address (or range) to a login account. This
	 * can be used to detect client logins for users without
	 * them having to login in.
	 * 
	 * @param connection
	 * @param personId
	 * @param ipAddress
	...
	private static void createIpLogin(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT login_ip_address.id FROM login_ip_address WHERE login_ip_address.person_id = ? AND login_ip_address.ip_address = ?");
		...
		statement = connection.prepareStatement("INSERT INTO login_ip_address (person_id, ip_address) VALUES (?, ?)");
	...
	static Type createRole(Connection connection,
	...
	public static Person createPerson(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT persons.id, persons.first_name, persons.last_name, persons.email FROM persons WHERE persons.email = ?");
		...
		if (table.getRowCount() == 1) {
			Person person = new Person(table.getInt(0, 0),
					table.getString(0, 1),
					table.getString(0, 2),
					table.getString(0, 3));
		...
		statement = connection.prepareStatement("INSERT INTO persons (public_uuid, first_name, last_name, affiliation, phone, address, email) VALUES (?, ?, ?, ?, ?, ?, ?)");
		...
		return createPerson(connection, firstName, lastName, affiliation,phone, address, email);
	}
	
	public static Person getPerson(Connection connection,
			int personId) throws SQLException {
		PreparedStatement statement = 
				connection.prepareStatement("SELECT persons.id, persons.first_name, persons.last_name FROM persons WHERE persons.id = ?");
	...
	public static void createLoginUser(Connection connection, 
		...
		PreparedStatement statement = 
				connection.prepareStatement("DELETE FROM login_persons WHERE login_persons.person_id = ?");
		try {
		...
		statement = connection.prepareStatement("INSERT INTO login_persons (person_id, user_name, password_hash_salted, salt, key) VALUES (?, ?, ?, ?, ?)");
	...
	public static List<Person> getGlobalPersons(Connection connection) throws SQLException {
		PreparedStatement statement = 
				connection.prepareStatement("SELECT persons.id, persons.first_name, persons.last_name FROM persons WHERE persons.user_type_id = 3 OR persons.user_type_id = 4");
		...
		try {
			...
			for (int i = 0; i < table.getRowCount(); ++i) {
				Person person = new Person(table.getInt(i, 0), 
						table.getString(i, 1), 
						table.getString(i, 2));
