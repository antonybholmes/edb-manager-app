...
public class GEO {
  /**
   * Creates a sample type such as Microarray to describe what the sample is.
   * 
   * @param connection
   * @param accession
   * @return
   ...
   */
  public static int createPlatform(Connection connection, String accession)
      throws SQLException {

    return createAccession(connection, "geo_platforms", accession);
  }

  public static int createSeries(Connection connection, String accession)
      throws SQLException {
  ...
  public static int createSample(Connection connection,
    ...
    if (geoSeriesId == -1 || geoPlatformId == -1 || sampleId == -1
        || accession == null || accession.length() == 0) {
      return -1;
    }

    PreparedStatement statement = connection.prepareStatement(
        "SELECT geo_samples.id FROM geo_samples WHERE geo_samples.name = ?");
    ...
    statement = connection.prepareStatement(
        "INSERT INTO geo_samples (geo_series_id, geo_platform_id, sample_id, name) VALUES (?, ?, ?, ?)");
  ...
  public static int createAccession(Connection connection,
      String type,
    ...
    String sql = new StringBuilder().append("SELECT ").append(type)
        .append(".id FROM ").append(type).append(" WHERE LOWER(").append(type)
        .append(".name) = LOWER(?)").toString();
