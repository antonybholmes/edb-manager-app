...
public class VFS {
  ...
  public static void createVFS(Connection connection)
    ...
    int exsVfsRootId = createVfsExDir(connection); // createVfsDir(connection,
                                                   // rootVfsId, "Experiments");

    statement = connection.prepareStatement(
        "SELECT experiments.id, experiments.public_id FROM experiments");
    ...
    for (int ei = 0; ei < experimentTable.getRowCount(); ++ei) {
      ...
      statement = connection.prepareStatement(
          "SELECT samples.id, samples.name FROM samples WHERE experiment_id = ?");
      ...
      for (int si = 0; si < vfsTable.getRowCount(); ++si) {
        ...
        statement = connection.prepareStatement(
            "SELECT files.name FROM files WHERE sample_id = ?");
    ...
    for (int ei = 0; ei < experimentTable.getRowCount(); ++ei) {
      ...
      for (int si = 0; si < vfsTable.getRowCount(); ++si) {
        ...
        statement = connection.prepareStatement(
            "SELECT directories.name FROM directories WHERE sample_id = ?");
        ...
        for (int fi = 0; fi < vfsTable2.getRowCount(); ++fi) {
          ...
          statement = connection.prepareStatement(
              "UPDATE vfs SET path = ? WHERE name = ? AND id = ?");
  ...
  public static int createSamplesDir(Connection c, int exVfsId)
      throws SQLException {
  ...
  public static void updatePath(Connection connection, int vfsId, String path)
      throws SQLException {
    PreparedStatement statement = connection
        .prepareStatement("UPDATE vfs SET path = ? WHERE id = ?");
  ...
  public static int createVfsEntry(Connection connection,
    ...
    PreparedStatement statement = connection.prepareStatement(
        "SELECT vfs.id FROM vfs WHERE name = ? AND parent_id = ? AND type_id = ?");
    ...
    statement = connection.prepareStatement(
        "INSERT INTO vfs (name, parent_id, type_id) VALUES (?, ?, ?)");
  ...
  public static void createSampleFileLink(Connection connection,
    ...
    PreparedStatement statement = connection.prepareStatement(
        "SELECT sample_files.id FROM sample_files WHERE sample_id = ? AND vfs_id = ?");
    ...
    if (!exists) {
      statement = connection.prepareStatement(
          "INSERT INTO sample_files (sample_id, vfs_id) VALUES (?, ?)");
  ...
  public static void createExperimentFileLink(Connection connection,
      ...
      int vfsId) throws SQLException {
    PreparedStatement statement = connection.prepareStatement(
        "SELECT experiment_files.id FROM experiment_files WHERE experiment_id = ? AND vfs_id = ?");
    ...
    if (!exists) {
      statement = connection.prepareStatement(
          "INSERT INTO experiment_files (experiment_id, vfs_id) VALUES (?, ?)");
   ...
   * field, String value) throws SQLException, ParseException {
   * PreparedStatement statement = connection.
   * prepareStatement("SELECT vfs_tags.id FROM vfs_tags WHERE vfs_tags.vfs_id = ? AND vfs_tags.tag_id = ?"
   * );
   * 
   * DatabaseResultsTable table;
   * 
   * try { statement.setInt(1, sampleId); statement.setInt(2, field.getId());
   * 
   ...
   * prepareStatement("INSERT INTO vfs_tags (vfs_id, tag_id, value) VALUES (?, ?, ?)"
   * );
   * 
   * try { statement.setInt(1, sampleId); statement.setInt(2, field.getId());
   ...
   * 
   * public static int createVfsField(Connection connection, int sampleId, Type
   * field, int value) throws SQLException, ParseException { PreparedStatement
   * statement = connection.
   * prepareStatement("SELECT vfs_int_tags.id FROM vfs_int_tags WHERE vfs_int_tags.vfs_id = ? AND vfs_int_tags.tag_id = ?"
   * );
   * 
   * DatabaseResultsTable table;
   * 
   ...
   * 
   * statement = connection.
   * prepareStatement("INSERT INTO vfs_int_tags (vfs_id, tag_id, value) VALUES (?, ?, ?)"
   * );
   * 
   * try { statement.setInt(1, sampleId); statement.setInt(2, field.getId());
   ...
   * 
   * 
   * public static void addVfsSearchTerms(Connection connection, int vfsId, Type
   * field, Set<String> keywords) throws SQLException, ParseException { for
   * (String keyword : keywords) { addVfsSearchTerm(connection, vfsId, field,
   * keyword); } }
   * 
   * public static void addVfsSearchTerm(Connection connection, int vfsId, Type
   * field, String keyword) throws SQLException, ParseException { //int
   * categoryId = Search.createSearchCategory(connection, path);
   * 
   * //Type field = Samples.createField(connection, path);
   * 
   * createVfsSearchKeyword(connection, vfsId, keyword, field);
   * 
   * // automatically add it to all addVfsSearchTermAll(connection, vfsId,
   * keyword); }
   * 
   * public static void addVfsSearchTermAll(Connection connection, int vfsId,
   * String keyword) throws SQLException, ParseException { //int categoryId =
   * Search.createSearchCategory(connection, path);
   * 
   ...
   * createVfsSearchKeyword(connection, vfsId, keyword, field); }
   * 
   * public static void createVfsSearchKeyword(Connection connection, int vfsId,
   * String keyword, Type field) throws SQLException, ParseException { int
   * keywordId = Search.createKeyword(connection, keyword);
   * 
   * createVfsSearch(connection, vfsId, keywordId, keyword, field); }
   * 
   * public static int createVfsSearch(Connection connection, int vfsId, int
   * keywordId, String keyword, Type field) throws SQLException, ParseException
   * {
   * 
   * int searchFieldKeywordId = Search.createSearchFieldKeyword(connection,
   * field, keywordId, keyword);
   * 
   * PreparedStatement statement = connection.
   * prepareStatement("SELECT vfs_search.id FROM vfs_search WHERE vfs_search.search_field_keyword_id = ? AND vfs_search.vfs_id = ?"
   * );
   * 
   * DatabaseResultsTable table;
   * 
   * try { statement.setInt(1, searchFieldKeywordId); statement.setInt(2,
   * vfsId);
   * 
   * table = JDBCConnection.getTable(statement); } finally { statement.close();
   * }
   ...
   * 
   * statement = connection.
   * prepareStatement("INSERT INTO vfs_search (search_field_keyword_id, vfs_id) VALUES (?, ?)"
   * );
   * 
   * try { statement.setInt(1, searchFieldKeywordId); statement.setInt(2,
   * vfsId);
   ...
   * 
   * public static void createVfsPermissions(Connection connection, int
   * personId) throws SQLException { List<Integer> ids =
   * JDBCConnection.getIntList(connection, FILES_SQL);
   * 
   * PreparedStatement statement = connection.
   * prepareStatement("DELETE FROM vfs_permissions WHERE vfs_permissions.person_id = ?"
   * );
   * 
   * try { statement.setInt(1, personId);
   * 
   * statement.execute(); } finally { statement.close(); }
   * 
   * statement = connection.
   * prepareStatement("INSERT INTO vfs_permissions (vfs_id, person_id) VALUES (?, ?)"
   * );
   * 
   * try { for (int id : ids) { statement.setInt(1, id); statement.setInt(2,
   * personId);
   * 
   * statement.execute(); } } finally { statement.close(); } }
   */
