...
public class Experiments {
	public static int[] createExperimentFromIdf(Connection connection,
		...
		int experimentId = Experiments.createExperiment(connection, 
				publicName, 
				name, 
				description);
		...
		//addExperimentSearchTerms(connection, experimentId, new Path("all_categories"), TextUtils.keywords(name));


		Type role = Persons.createRole(connection, roleName);

		//int fileId = Files.createExperimentFile(connection, dataDirectory, experimentId, "expression.rma");

		//fileId = Files.createExperimentFile(connection, dataDirectory, experimentId, "expression.mas5");
	...
	public static Species createOrganism(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT organisms.id, organisms.name, organisms.scientific_name FROM organisms WHERE organisms.name = ?");
		...
		if (table.getRowCount() == 1) {
			return new Species(table.getInt(0, 0), table.getString(0, 1), table.getString(0, 2));
		}
		
		statement = 
				connection.prepareStatement("INSERT INTO organisms (name, scientific_name) VALUES (?, ?)");
	...
	public static Species getOrganism(Connection connection,
			String scientificName) throws SQLException {
		PreparedStatement statement = 
				connection.prepareStatement("SELECT organisms.id, organisms.name, organisms.scientific_name FROM organisms WHERE organisms.scientific_name = ?");
	...
	public static int createExperiment(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT experiments.id FROM experiments WHERE experiments.name = ? OR experiments.public_id = ?");
		...
		statement = connection.prepareStatement("INSERT INTO experiments (public_id, name, description) VALUES (?, ?, ?)");
	 ...
	 */
	public static String createExperimentPublicId(Connection connection, int personId) throws SQLException, ParseException {
		PreparedStatement statement = 
				connection.prepareStatement("SELECT LOWER(persons.first_name) FROM persons WHERE persons.id = ?");
		...
		statement = 
				connection.prepareStatement("SELECT MAX(experiments.id) FROM experiments");
	...
	public static Experiment getExperiment(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT experiments.id, experiments.public_id, experiments.name, experiments.description FROM experiments WHERE experiments.id = ?");
		...
		try {
			...
			if (table.getRowCount() == 1) {
				experiment = new Experiment(table.getInt(0, 0), 
						table.getString(0, 1), 
						table.getString(0, 2), 
						table.getString(0, 3),
						null);
