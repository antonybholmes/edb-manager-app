...
public class Keywords {
	public static void keywordIndexExperiment(Connection connection,
		...
		for (String keyword : keywords) {
			...
			PreparedStatement statement = 
					connection.prepareStatement("SELECT keywords_experiments.id FROM keywords_experiments WHERE keywords_experiments.keyword_id = ? AND keywords_experiments.experiment_id = ? AND keywords_experiments.field_id = ?");
			...
			statement = connection.prepareStatement("INSERT INTO keywords_experiments (keyword_id, experiment_id, field_id) VALUES (?, ?, ?)");
	...
	public static void keywordIndexSample(Connection connection,
		...
		for (String keyword : keywords) {
			...
			PreparedStatement statement = 
					connection.prepareStatement("SELECT keywords_samples.id FROM keywords_samples WHERE keywords_samples.keyword_id = ? AND keywords_samples.sample_id = ? AND keywords_samples.field_id = ?");
			...
			statement = connection.prepareStatement("INSERT INTO keywords_samples (keyword_id, experiment_id, sample_id, field_id) VALUES (?, ?, ?, ?)");
	...
	/*
	public static int createField(Connection connection,
			String field) throws SQLException {
		PreparedStatement statement = 
				connection.prepareStatement("SELECT fields.id FROM fields WHERE fields.name = ?");
		
		DatabaseResultsTable table;
		
		try {
			statement.setString(1, field.toLowerCase());
		...
		statement = connection.prepareStatement("INSERT INTO fields (name) VALUES (?)");
