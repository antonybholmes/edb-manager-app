...
import org.abh.common.sys.SysUtils;

import edu.columbia.rdf.edb.http.DatabaseServlet;

public class Samples {
	private static final String JSON_SAMPLE_FIELDS_SQL =
			"INSERT INTO json_sample_fields (sample_id, json) VALUES ('a1', 'a2')";

	private static final String JSON_SAMPLE_GEO_SQL =
			"INSERT INTO json_sample_geo (sample_id, json) VALUES ('a1', 'a2')";

	private static final String JSON_SAMPLE_PERSONS_SQL =
			"INSERT INTO json_sample_persons (sample_id, json) VALUES ('a1', 'a2')";


	public static Type createExpressionType(Connection connection,
	...
	public static void keywordIndexAttribute(Connection connection, 
		...
		for (String keyword : keywords) {
			int keywordId = CreateExperiment.createKeyword(connection, keyword);

			PreparedStatement statement = 
					connection.prepareStatement("SELECT keyword_attributes.id FROM keyword_attributes WHERE keyword_attributes.keyword_id = ? AND keyword_attributes.attribute_id = ?");
			...
			try {
				...
				table = MySQLJDBCDatabase.create(statement);
			} finally {
				statement.close();
			}
			...
			statement = connection.prepareStatement("INSERT INTO keyword_attributes VALUES (DEFAULT, ?, ?)");
	...
	public static int createSamplePerson(Connection connection,
			int sampleId,
			int personId,
			Type role) throws SQLException, ParseException {
		PreparedStatement statement = 
				connection.prepareStatement("SELECT sample_persons.id FROM sample_persons WHERE sample_persons.sample_id = ? AND sample_persons.person_id = ? AND sample_persons.role_id = ?");
		...
		statement = connection.prepareStatement("INSERT INTO sample_persons (sample_id, person_id, role_id) VALUES (?, ?, ?)");
	...
	public static int createSampleTag(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT tags_sample.id FROM tags_sample WHERE tags_sample.sample_id = ? AND tags_sample.tag_id = ?");
		...
		statement = connection.prepareStatement("INSERT INTO tags_sample (sample_id, tag_id, value) VALUES (?, ?, ?)");
	...
	public static int createSampleTag(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT tags_sample_int.id FROM tags_sample_int WHERE tags_sample_int.sample_id = ? AND tags_sample_int.tag_id = ?");
		...
		statement = connection.prepareStatement("INSERT INTO tags_sample_int (sample_id, tag_id, value) VALUES (?, ?, ?)");
	...
	public static int createSampleTag(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT tags_sample_float.id FROM tags_sample_float WHERE tags_sample_float.sample_id = ? AND tags_sample_float.tag_id = ?");
		...
		statement = connection.prepareStatement("INSERT INTO tags_sample_float (sample_id, tag_id, value) VALUES (?, ?, ?)");
	...
	public static int createSample(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT samples.id FROM samples WHERE samples.name = ? AND samples.experiment_id = ?");
		...
		statement = connection.prepareStatement("INSERT INTO samples (experiment_id, expression_type_id, name, organism_id) VALUES (?, ?, ?, ?)");
		...
		return createSample(connection, experimentId, expressionTypeId, name, organismId, releaseDate);
	}

	/**
	 * Each sample needs an alias which we default to its name,
	 * Other aliases may be added.
	 * 
	 * @param connection
	 * @param sampleId
	 * @param name
	...
	public static int createAlias(Connection connection,
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT sample_aliases.id FROM sample_aliases WHERE sample_aliases.sample_id = ?");
		...
		statement = connection.prepareStatement("INSERT INTO sample_aliases (sample_id, name) VALUES (?, ?)");
	...
	public static int createGEO(Connection connection, 
		...
		PreparedStatement statement = 
				connection.prepareStatement("SELECT samples_geo.id FROM samples_geo WHERE samples_geo.sample_id = ?");
		...
		statement = connection.prepareStatement("INSERT INTO samples_geo (sample_id, geo_series_accession, geo_accession, geo_platform) VALUES (?, ?, ?, ?)");
		...
		return createGEO(connection, sampleId, seriesAccession, sampleAccession, platform);
	}
	...
	public static void sampleJson(Connection connection) throws SQLException {
		...
		PreparedStatement statement = 
				connection.prepareStatement("delete from json_sample_fields");
		...
		List<Integer> sampleIds = 
				edu.columbia.rdf.edb.http.Samples.getAllSampleIds(connection);
	...
	public static void sampleGeoJson(Connection connection) throws SQLException {
		PreparedStatement statement = 
				connection.prepareStatement("delete from json_sample_geo");
	...
	public static void samplePersonsJson(Connection connection) throws SQLException {
		PreparedStatement statement = 
				connection.prepareStatement("delete from json_sample_persons");
